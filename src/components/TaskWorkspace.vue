    ...
    <div id="workspaceContent" class="workspace-content" hidden>
      ...
      <section id="worklogPanel" class="workspace-panel worklog-panel" role="tabpanel" aria-labelledby="worklogTab" hidden>
        <div class="editor-section" id="descriptionSection">
          <div class="workspace-section-heading"><h3>长期描述</h3><button class="icon-button subtle section-collapse-toggle" id="collapseDescription" type="button" title="折叠长期描述" aria-label="折叠长期描述" aria-pressed="false"><i data-lucide="chevron-down"></i></button><button class="icon-button subtle section-zoom" id="zoomDescription" type="button" title="放大长期描述" aria-label="放大长期描述" aria-pressed="false"></button><span id="descriptionSaveStatus" class="save-status" aria-live="polite">已保存</span><button id="descriptionRetry" class="button text-button" type="button" hidden>重试</button></div>
