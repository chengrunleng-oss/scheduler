    ...
    <div id="workspaceContent" class="workspace-content" hidden>
      ...
      <section id="worklogPanel" class="workspace-panel worklog-panel" role="tabpanel" aria-labelledby="worklogTab" hidden>
        <div class="editor-section" id="descriptionSection">
          <div class="workspace-section-heading"><button class="icon-button subtle section-collapse-toggle" id="collapseDescription" type="button" title="折叠长期描述" aria-label="折叠长期描述" aria-pressed="false"><i data-lucide="chevron-down"></i></button><h3>长期描述</h3><button class="icon-button subtle section-zoom" id="zoomDescription" type="button" title="放大长期描述" aria-label="放大长期描述" aria-pressed="false"></button><button class="button text-button" id="exportDescriptionPdf" type="button" title="导出长期描述为 PDF"><i data-lucide="download"></i><span>导出 PDF</span></button><span id="descriptionSaveStatus" class="save-status" aria-live="polite">已保存</span><button id="descriptionRetry" class="button text-button" type="button" hidden>重试</button></div>
          <div class="workspace-section-body"><div id="descriptionEditor" class="markdown-editor" aria-label="长期描述编辑器"></div></div>
        </div>
        <div class="editor-section daily-section" id="dailySection">
          <div class="workspace-section-heading daily-heading"><button class="icon-button subtle section-collapse-toggle" id="collapseDaily" type="button" title="折叠每日记录" aria-label="折叠每日记录" aria-pressed="false"><i data-lucide="chevron-down"></i></button><h3>每日记录</h3><button class="icon-button subtle section-zoom" id="zoomDaily" type="button" title="放大每日记录" aria-label="放大每日记录" aria-pressed="false"></button><button class="button text-button" id="exportWorklogPdf" type="button" title="导出当前每日记录为 PDF"><i data-lucide="download"></i><span>导出 PDF</span></button><button id="newWorklog" class="button secondary compact-button" type="button"><i data-lucide="circle-plus"></i><span>新建记录</span></button><label class="compact-field"><span class="sr-only">工作日期</span><input id="worklogDate" type="date" /></label><label class="progress-field"><span>进度</span><input id="worklogProgress" type="number" min="0" max="100" step="1" inputmode="numeric" /><span>%</span></label></div>
          <div class="workspace-section-body"><div class="editor-save-line"><span id="worklogSaveStatus" class="save-status" aria-live="polite">已保存</span><button id="worklogRetry" class="button text-button" type="button" hidden>重试</button></div>
          <div id="worklogEditor" class="markdown-editor" aria-label="每日工作记录编辑器"></div></div>
        </div>
        <section class="worklog-history-section" id="worklogHistorySection" aria-labelledby="worklogHistoryTitle"><div class="workspace-section-heading"><button class="icon-button subtle section-collapse-toggle" id="collapseHistory" type="button" title="折叠历史记录" aria-label="折叠历史记录" aria-pressed="false"><i data-lucide="chevron-down"></i></button><h3 id="worklogHistoryTitle">历史记录</h3><button class="icon-button subtle section-zoom" id="zoomHistory" type="button" title="放大历史记录" aria-label="放大历史记录" aria-pressed="false"></button></div><nav id="historyZoomToc" class="history-zoom-toc" aria-label="历史记录日期目录" hidden></nav><div id="worklogUndo" class="worklog-undo" role="status" hidden><span id="worklogUndoText"></span><button id="undoWorklogDelete" class="button text-button" type="button">撤销删除</button></div><div id="worklogHistory" class="worklog-history"></div></section>
      </section>
