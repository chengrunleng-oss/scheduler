...
function renderFolderTree(state: AppState, container: HTMLElement): void {
  ...
  container.append(
    createScopeRow("all", "全部任务", state.tasks.length, state.preferences.folderScope === "all"),
    createScopeRow("root", "未分类", state.tasks.filter((task) => task.folderId === null).length, state.preferences.folderScope === "root"),
  );
  ...
  const appendFolder = (folder: Folder, depth: number) => {
    ...
    actions.append(
      createFolderAction("add-child-folder", folder.id, "＋", "新建子文件夹", getFolderDepth(state.folders, folder.id) >= MAX_FOLDER_DEPTH),
      createFolderAction("edit-folder", folder.id, "✎", "编辑文件夹"),
      createFolderAction("delete-folder", folder.id, "×", "删除文件夹"),
    );
...
function createScopeRow(scope: "all" | "root", label: string, count: number, selected: boolean): HTMLElement {
...
function createFolderAction(action: string, folderId: string, text: string, label: string, disabled = false): HTMLButtonElement {
...
function renderTreeView(state: AppState, visibleTasks: Task[], view: ViewState, container: HTMLElement): void {
  ...
  const appendTasks = (folderId: string | null, depth: number) => {
  ...
  const appendFolder = (folder: Folder, depth: number) => {
    const descendantIds = getFolderDescendantIds(state.folders, folder.id);
    const taskCount = visibleTasks.filter(
      (task) => task.folderId === folder.id || Boolean(task.folderId && descendantIds.has(task.folderId)),
    ).length;
    if (!showEmptyFolders && taskCount === 0) return;
    ...
    if (folder.collapsed && !view.query) return;
    appendTasks(folder.id, depth + 1);
    for (const child of childrenByParent.get(folder.id) ?? []) appendFolder(child, depth + 1);
  };
  ...
  if (state.preferences.folderScope !== "all") {
    ...
    if (selectedFolder) appendFolder(selectedFolder, 0);
    return;
  }
...
function renderPriorityView(state: AppState, tasks: Task[], view: ViewState, container: HTMLElement): void {
  const groups: Array<[Priority, string]> = [
    ["high", "高优先级"],
    ["medium", "中优先级"],
    ["low", "低优先级"],
  ];
...
function renderDueDateView(state: AppState, tasks: Task[], view: ViewState, container: HTMLElement): void {
  const groups = [
    ["overdue", "已逾期"],
    ["today", "今天"],
...
function createTaskActions(task: Task): HTMLElement {
  const actions = createElement("div", { className: "task-actions" });
  if (task.status === "active") {
    actions.append(
      createTaskAction("complete", "✓", "移至已完成"),
      createTaskAction("discard", "∅", "移至不再需要"),
    );
  } else {
    actions.append(createTaskAction("restore", "↺", "恢复为待办"));
  }
  actions.append(
    createTaskAction("raise-priority", "↑", "提高优先级", task.priority === "high"),
    createTaskAction("lower-priority", "↓", "降低优先级", task.priority === "low"),
    createTaskAction("delete", "×", "删除任务", false, true),
  );
  return actions;
}

function createTaskAction(action: string, text: string, label: string, disabled = false, danger = false): HTMLButtonElement {
...
function formatDueDate(task: Task): string {
  ...
  if (task.status === "active") {
    ...
    if (group === "overdue") return `逾期 · ${label}`;
    if (group === "today") return "今天截止";
  }
  return `${label} 截止`;
}
