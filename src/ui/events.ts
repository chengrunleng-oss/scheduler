...
export function bindEvents(els: Elements, store: AppStore, dialogs: Dialogs, requestRender: () => void): EventBindings {
  function readTaskDraft(): TaskDraft {
    return {
  ...
  els.filters.forEach((button) => {
    button.addEventListener("click", () => {
      store.dispatch({ type: "set-filter", filter: (button.dataset.filter || "all") as TaskFilter });
    });
  });
  ...
  els.taskList.addEventListener("change", (event) => {
    ...
    const taskId = target.closest<HTMLElement>(".task-item")?.dataset.id;
    if (!taskId) return;
  ...
  els.taskList.addEventListener("click", async (event) => {
    ...
    if (!button) return;
    const taskId = button.closest<HTMLElement>(".task-item")?.dataset.id;
    ...
    if (button.dataset.action === "delete") {
      ...
      if (confirmed) store.dispatch({ type: "delete-task", id: task.id });
      return;
    }
  ...
  function handleChecklistEvent(event: Event, kind: ChecklistKind): void {
    const row = (event.target as HTMLElement).closest<HTMLElement>(".check-row");
