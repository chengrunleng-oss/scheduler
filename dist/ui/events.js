import { coercePriority, getFolderDescendantIds, normalizeMultiline, normalizeText, selectVisibleTasks, toISODate, } from "../domain.js";
...
export function bindEvents(els, store, dialogs, requestRender) {
    ...
    function currentFolderForNewTask() {
    ...
    async function prepareSelectedTaskMutation(taskId) {
        if (selectedTaskId !== taskId || !detailDirty)
            return true;
    ...
    els.statusFilters.forEach((button) => {
        button.addEventListener("click", () => {
            store.dispatch({ type: "set-status-filter", filter: (button.dataset.filter || "all") });
        });
    });
    els.viewModes.forEach((button) => {
        button.addEventListener("click", () => {
            const viewMode = (button.dataset.view || "tree");
            store.dispatch({ type: "set-view-mode", viewMode });
            const defaultSort = viewMode === "tree" ? "manual" : viewMode === "priority" ? "due_date" : "priority";
            store.dispatch({ type: "set-sort-mode", sortMode: defaultSort });
        });
    });
    els.sortMode.addEventListener("change", () => {
        store.dispatch({ type: "set-sort-mode", sortMode: els.sortMode.value });
    });
    ...
    els.taskList.addEventListener("click", async (event) => {
        ...
        const folderButton = target.closest("button[data-folder-id]");
        ...
        if (!task || !(await prepareSelectedTaskMutation(task.id)))
            return;
        ...
        if (action === "complete")
            store.dispatch({ type: "set-task-status", id: task.id, status: "completed" });
        if (action === "discard")
            store.dispatch({ type: "set-task-status", id: task.id, status: "discarded" });
        if (action === "restore")
            store.dispatch({ type: "set-task-status", id: task.id, status: "active" });
        if (action === "raise-priority")
            store.dispatch({ type: "adjust-task-priority", id: task.id, direction: "raise" });
        if (action === "lower-priority")
            store.dispatch({ type: "adjust-task-priority", id: task.id, direction: "lower" });
    });
    els.taskList.addEventListener("keydown", async (event) => {
        ...
        if (event.key !== "ArrowDown" && event.key !== "ArrowUp")
            return;
        event.preventDefault();
        const rows = Array.from(els.taskList.querySelectorAll(".task-item"));
    ...
    els.newFolder.addEventListener("click", async () => {
        ...
        if (draft)
            store.dispatch({ type: "add-folder", draft });
    });
    els.folderTree.addEventListener("click", async (event) => {
        ...
        if (button.dataset.action === "select-folder" && folderId) {
            store.dispatch({ type: "set-folder-scope", folderScope: folderId });
        ...
        if (button.dataset.action === "edit-folder") {
            ...
            if (draft)
                store.dispatch({ type: "update-folder", id: folder.id, draft });
        }
        if (button.dataset.action === "delete-folder") {
            ...
            if (nonEmpty) {
                ...
                if (strategy)
                    store.dispatch({ type: "delete-folder", id: folder.id, strategy });
            }
            else {
