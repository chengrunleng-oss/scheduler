...
`,u=(e=>(e.CodeMirror=`code-mirror`,e.ListItem=`list-item`,e.LinkTooltip=`link-tooltip`,e.Cursor=`cursor`,e.ImageBlock=`image-block`,e.BlockEdit=`block-edit`,e.Toolbar=`toolbar`,e.Placeholder=`placeholder`,e.Table=`table`,e.Latex=`latex`,e.TopBar=`top-bar`,e.AI=`ai`,e))(u||{}),d=(e,i)=>{e.config(a(u.LinkTooltip)).config(r).config(e=>{e.update(t.key,e=>({...e,linkIcon:i?.linkIcon??s,editButton:i?.editButton??c,removeButton:i?.removeButton??l,confirmButton:i?.confirmButton??o,inputPlaceholder:i?.inputPlaceholder??`Paste link...`,onCopyLink:i?.onCopyLink??(()=>{})}))}).use(n)};export{d as linkTooltip};